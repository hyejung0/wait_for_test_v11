...
\#tab:tbl-target-therapy-mutation-status-contingency-table-v2 
2 by 2 contingency table for targetable mutation status and the respective treatment choices among the wait patients.
```{r results='asis'}
#| echo: false
#| eval: true
#| warning: false
...
`r tmp1` reported high SMD, ranging from `r min(SMD_sw_report)` to `r max(SMD_sw_report)` in the IPW population (\@ref(tab:tbl-primary-wait-1L-dscriptive-both-v2)). Thus, IPW failed to generate pseudopopulations where the potential confounders are evenly distributed. 
