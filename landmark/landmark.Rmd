...
Out of total 85,572 patients in the data set, there are 371 patients who have missing data for time to death or censoring. These patienst are excluded first, making the original sample size to be 84,207.
...
# ptData1_wait<-ptData1[cohort=="wait",]
...
ptData1_wait_out<-whole_analysis_func(ptData1_wait)
ptData1_wait_out$KM_curves
ptData1_wait_out$SMD_table 
#Some are having SMD > 0.1. But all are less than 0.2. So just use it. Report it.
# Histology, gender, race/ethinicity and age at diagnosis are 
# Save and report 
...
With IPW, `r tmp1` reported slightly high SMD, ranging from `r min(SMD_sw_report)` to `r max(SMD_sw_report)`. Since they don't exceed 0.2, we will just use IPW population, assuming that covariance balanced was achieved. Below is KM plot of the IPW population.  The median survival (95% CI) of those who received targeted therapy was `r  ptData1_wait_out$median_survivals$yes_ipw$"cohort=Target therapy"`, while those who didn't were  `r ptData1_wait_out$median_survivals$yes_ipw$"cohort=something else"`.
...
ptData1_wait_out$KM_curves$yes_ipw+ggtitle(label=NULL)
...
ptData2[,]
ptData2[,valid_test_date:=pmin(earliest_valid_ResultDate__on_after_time0_PDL1,earliest_valid_ResultDate__on_after_time0_NoPDL1,na.rm = T)]

tmp<-ptData2[cohort=="no_wait" & FirstLineTherapyGroup_old_and_new=="Target",.(PatientID,AdvancedDiagnosisDate,valid_test_date, StartDate)]
...
We observed positivity assumption violation in some confounders in the ECOG 3-4, Asian, and Never-smoker subgroups.  Thus, we do canoot perform causal analysese for these three subgroups. 
...
gt(subgroup_analyses_primary$`ECOG (3-4)`$SMD_table) |> 
  ...
  # optional styling:
  opt_align_table_header("left") |> 
  tab_caption(caption = md("Table XX. Summary of Patient Characteristics at Baseline for ECOG 3-4 subgroup, without IPW weighting. Primary analysis was applied")) |> 
  as_raw_html()  #this will display the table on

```

```{r tbl-subgroup-primary-asian}
#| echo: false
#| eval: false
#| warning: false
#| message: false
...
gt(subgroup_analyses_primary$Asian$SMD_table) |> 
  ...
  # optional styling:
  opt_align_table_header("left") |> 
  tab_caption(caption = md("Table XX. Summary of Patient Characteristics at Baseline for Asian subgroup. Primary analysis was applied.")) |> 
  as_raw_html()  #this will display the table on

```

```{r tbl-subgroup-primary-never-smoked}
#| echo: false
#| eval: false
#| warning: false
#| message: false
...
# build gt table
gt(subgroup_analyses_primary$`Never Smoked`$SMD_table) |> 
  # First two columns stay as-is:
  cols_label(
    this.rownames = "Variable",
    level         = "Level",
  ...
  # optional styling:
  opt_align_table_header("left") |> 
  tab_caption(caption = md("Table XX. Summary of Patient Characteristics at Baseline for Never-smoked subgroup. Primary analysis was applied.")) |> 
  as_raw_html()  #this will display the table on
...
tmp[`Patient Population` %in% c("ECOG (3-4)","Asian","Never Smoked"), `Primary inclusion criteria`:=""]
...
When applying the primary inclusion criteria, either positivity assumption violation or large SMD was observed, rendering causal inference infeasible. In contrast, the data defined by the secondary inclusion criteria demonstrated no positivity violations or covariate imbalance from 2014 onward. Accordingly, we present below the causal estimates for 2014–2022, based on the secondary‐criteria dataset.
...
#| warning: false
#| message: false
#| label: fig-subgroup-by_year-secondary
#| fig-cap: Figure XX. HR(95%CI) of those who don't wait for valie test result prior to initiating 1L therapy, grouped by advanced diagnosis year. Samples were collected using secondary inclusion criteria.
...
secondary_plot<-secondary_plot[year>=2014,] #Only show for just 2014 and onward.
#Secondary analysis
