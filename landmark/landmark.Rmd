...
**Q**: In the Secondary analysis of the whole population, there are people in "no_wait" cohort who received targetable therapy. Is this just data error? Or is there some particular demographics that demostrates this?
...
#| eval: true
#| echo: false
#| label: tbl-secondary-target-investigate-copy
...
tbl_summary(
  ...
  #   subtitle = md("`gtcars` is an R dataset")
  # ) |>
  tab_caption(caption = md("Table XX. Summary of patient characteristics at baseline. Secondary inclusion, no-wait cohort. Stratified by their respective 1L therapy.")) |> 
  as_raw_html() 
```


Based on the table above, it seems like among the patients patients who waited to have valid therapy results and either didn't receive any therapy or received something else than target therapy have:
...
compared to those who waited for valid therapy results and received targeted therapy.
...
**Q**: Under the Aim 1, where I conduct the analysis using primary criteria, I showed KM curve of those who received target therapy vs those who either received something else or didn't receive anything  under primary inclusion criteria, I show KM curve of those who received targetable therapy vs not (including those who didn’t) \@ref(fig:fig-secondary-KMplot). Please rebuild by 
...
```{r target-therapy-mutation-status-contingency-table-v2}
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
To build IPW, I thought it would be more appropriate to find the values around the time the person received 1L therapy (Among those who received 1L therapy). But among those who received 1L therapy, 8 people didn't have any record of ECOG between 2 months before and up to 1L therapy date and 77 people didn't have any albumin observed 6 weeks prior to and up to 1L theray date. So we decided to just drop all time-varying variables: ECOG, albumin, weight change. And we used demographic (fixed values) and whethe or not they had targetable mutation identified.  

With IPW, `r tmp1` reported high SMD, ranging from `r min(SMD_sw_report)` to `r max(SMD_sw_report)`. Thus, the covariate balance was really off. Below we present the table of covariate distribution before and after IPW.


```{r tbl-primary-wait-1L-dscriptive-both-v2}
#| echo: false
#| eval: true
#| warning: false
#| message: false
...
## Find out patterns of missing for lab values

**Q**: The exclusion tables \@ref{tab:tbl-exclusion_primary} and \@ref{tab:tbl-exclusion_secondary} show that we are losing more than half of the patient samples due to missing variables. This is a huge loss. Thus, we want to find ways to impute these values. Thus we decided look for patterns of missing.


**A**: Since we have longitudinal data set, we will look at pattern of missing in two levels:
...
temp<-merge.data.table(ptData_missing[,.(PatientID,AdvancedDiagnosisDate)],
 ...
 ### Entry Criteria - start ###
  ...
  #generate two
  ECOG_tmp<-copy(temp)
  ECOG_tmp[,min_date:=as.IDate(as.Date(AdvancedDiagnosisDate)%m-% months(2))]
...
#6 months prior to 1 week or 4 weeks after.
weight_tmp<-copy(temp)
weight_tmp[,min_date:=as.IDate(as.Date(AdvancedDiagnosisDate)%m-% months(6))]
...
setnames(all_obs_primary,"EcogValue","ECOG")
all_obs_secondary<-
  merge(all_obs_secondary,
...
setnames(all_obs_secondary,"EcogValue","ECOG")
...
setnames(all_obs_primary,"TestResultCleaned","weight")
...
setnames(all_obs_secondary,"TestResultCleaned","weight")


saveRDS(all_obs_primary,"all_obs_primary.rds")
saveRDS(all_obs_secondary,"all_obs_secondary.rds")
...
all_obs_primary<-readRDS("all_obs_primary.rds")
all_obs_secondary<-readRDS("all_obs_secondary.rds")

all_obs_primary_missing<-copy(all_obs_primary)
all_obs_primary_missing[,c("PatientID","Date","time0"):=.(NULL)]

all_obs_secondary_missing<-copy(all_obs_secondary)
all_obs_secondary_missing[,c("PatientID","Date","time0"):=.(NULL)]
...
auxilliary<-colnames(all_obs_primary)[!colnames(all_obs_primary) %in%c("PatientID","Date","time0",VOI)]
...
count_observed<-all_obs_primary[,sum(!is.na(.SD)),by=PatientID,.SDcols=c(VOI,auxilliary)]
...
one_obs_primary<-all_obs_primary[PatientID=="FC85B4F31A510",.SD,.SDcols=c(VOI,auxilliary)]



count_observed<-all_obs_secondary[,sum(!is.na(.SD)),by=PatientID,.SDcols=c(VOI,auxilliary)]
count_observed<-count_observed[order(V1,decreasing = T),]  
count_observed[1,] #F94D0B13A35B4
#F94D0B13A35B4 has the largest number of observation.
#Save this person to work with.
one_obs_secondary<-all_obs_secondary[PatientID=="F94D0B13A35B4",.SD,.SDcols=c(VOI,auxilliary)]
...
# install.packages(c("naniar", "VIM", "mice"))
library(naniar)    # very nice ggplot‐based missing‐data plots
library(VIM)       # classic aggr() plots
...
all_obs_primary[,index:=1:.N,by=PatientID]
all_obs_primary_long<-
melt(all_obs_primary,id.vars = c("PatientID","index"),measure.vars=c(VOI,auxilliary), variable.name = "var", value.name = "val")
...
ggplot(population_summary_primary[var %in% VOI,],
  ...
  theme_minimal()+
  ggtitle("B) Primary inclusion criteria")
...
all_obs_secondary[,index:=1:.N,by=PatientID]
all_obs_secondary_long<-
  melt(all_obs_secondary,id.vars = c("PatientID","index"),measure.vars=c(VOI,auxilliary), variable.name = "var", value.name = "val")
...
sub_level_missing_primary1<-all_obs_primary[,lapply(.SD,function(x)mean(is.na(x))),.SDcols = VOI,by=PatientID]

sub_level_missing_secondary1<-all_obs_secondary[,lapply(.SD,function(x)mean(is.na(x))),.SDcols = VOI,by=PatientID]
...
primary_voi_missing_binary<-all_obs_primary[,lapply(.SD,function(x)ifelse(is.na(x),1,0)),.SDcols = VOI]
primary_voi_missing_binary<-cbind(all_obs_primary[,.(PatientID,index)],primary_voi_missing_binary)

secondary_voi_missing_binary<-all_obs_secondary[,lapply(.SD,function(x)ifelse(is.na(x),1,0)),.SDcols = VOI]
secondary_voi_missing_binary<-cbind(all_obs_secondary[,.(PatientID,index)],secondary_voi_missing_binary)



primary_aux_missing_binary<-all_obs_primary[,lapply(.SD,function(x)ifelse(is.na(x),1,0)),.SDcols = auxilliary]
primary_aux_missing_binary<-cbind(all_obs_primary[,.(PatientID,index)],primary_aux_missing_binary)

secondary_aux_missing_binary<-all_obs_secondary[,lapply(.SD,function(x)ifelse(is.na(x),1,0)),.SDcols = auxilliary]
secondary_aux_missing_binary<-cbind(all_obs_secondary[,.(PatientID,index)],secondary_aux_missing_binary)
...
### Numeric check: point-biserial correlations

You can turn each VOI’s missingness into a 0/1 indicator, then compute correlations with each auxiliary.

Interpretation: values near zero suggest the VOI is missing at random w.r.t. that auxiliary; large positive/negative values indicate a relationship you may want to account for in the imputation model.

```{r numeric-check-missing}
#| echo: false
#| eval: false
#| warning: false
#| message: false
...
# 2a) Define your VOIs
VOIs   <- c("Y1","Y2","Y3")
aux    <- setdiff(names(df), VOIs)

# 2b) Create NA‐indicators
df2 <- df %>%
  mutate(across(
    all_of(VOIs),
    ~ as.integer(is.na(.x)),
    .names = "{.col}_NA"       # yields Y1_NA, Y2_NA, Y3_NA
  ))

NA_inds <- paste0(VOIs, "_NA")

# 2c) Compute the correlation matrix
M <- cor(
  df2 %>% select(all_of(c(NA_inds, aux))),
  use = "pairwise.complete.obs"
)

# 2d) Extract just the VOI‐NA vs auxiliary block
M[NA_inds, aux]  # a 3×17 matrix of point‐biserial correlations
...
### A more formal check: logistic regression

A small logistic model can flag which auxiliaries are significant predictors of missingness:
