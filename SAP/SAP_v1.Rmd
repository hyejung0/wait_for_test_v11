...
-   A_k = { 1 if A_(k-1)=1 or if 1L therapy is initiated before C_k
      { 0 if A_(k-1)≠1 and if 1L therapy is not initiated before C_k
...
This assumption will be true if for all prognostic factors for $_{\bar{a}}(t+1) $
...
digraph SMART {
  ...
  FirstRandom  [label = 'Check is valid test is back']
  T1           [label = 'If back \\n (Responder)']
  T2           [label = 'If not back']
  T2_NotTreated       [label = 'Not treated \\n (Non responder)']
  T2_Treated        [label = 'Treated \\n (Responder)']

  Response_ET  [label = 'ET']
  Response_W    [label = 'W']
  
  SecondRandom [label = 'Second Stage Randomization']
  T3        [label = 'If back \\n (Responder)']
  T4      [label = 'If not back']
  T4_NotTreated       [label = 'Not treated \\n (Non responder)']
  T4_Treated        [label = 'Treated \\n (Responder)']

  Response_W_T3   [label = 'ET']
  Response_ET_T4     [label = 'W']  
  ...
  T2 -> T2_NotTreated
  T2 -> T2_Treated
  T2_Treated -> Response_ET
  
  # Force SecondRandom to be to the right of Response_ET
  Response_ET -> SecondRandom [style = invis, minlen = 2];
...
```{r SMART-diag, out.width='100%',  fig.cap="Figure XX: SMART diagram. The target trial I am trying to emulate"}
#| eval: true

knitr::include_graphics("./image/smart_diagram.png")
